    ...
    if request.method == 'POST':
        ...
        if proveedores: 
            for a in proveedores:
                proyect_save.proveedor.add(a) #guarda muchos a mucho
...
def ejemplos_proyect_edit(request,proyecto_id):
    ...
    proveedor_data = Proveedor.objects.filter(proyecto__id = proyect_data.id)
    proveedor_data_list = []
    ...
    return render(request,template_name,{'proyect_data':proyect_data,'proveedor_data_list':proveedor_data_list,'autor_list':proveedor_list})
...
def ejemplos_proyect_edit_save(request):
    if request.method == 'POST':
        ...
        Proyecto.objects.filter(pk=id).update(descripcion = descripcion)
        
        proyecto = Proyecto.objects.get(pk=id)#carga el proyecto que usaremos para muchos a muchos
        #actualizmos autores
        proveedor_data = Proveedor.objects.filter(proyecto__id = id)
        for a in proveedor_data:
            proyecto.proveedor.remove(a) #elimina muchos a mucho
        if proveedores: 
            for a in proveedores:
                proyecto.proveedor.add(a) #guarda muchos a mucho        
        messages.add_message(request, messages.INFO, 'proyecto creado')   
        return redirect('ejemplos_proyect_list')
    else:
        messages.add_message(request, messages.INFO, 'Error al crear el proyecto')   
